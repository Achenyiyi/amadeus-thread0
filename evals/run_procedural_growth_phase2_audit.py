...
SMOKES_JSON_RE = re.compile(r"^\[procedural-growth-phase2-smokes\]\s+json=(.+)$", re.MULTILINE)
SMOKES_MD_RE = re.compile(r"^\[procedural-growth-phase2-smokes\]\s+md=(.+)$", re.MULTILINE)
SMOKES_STATUS_RE = re.compile(r"^\[procedural-growth-phase2-smokes\]\s+overall_status=(.+)$", re.MULTILINE)
    ...
    return [
        {
            ...
            "title": "Procedural Planning Unit Tests",
            "command": _python("-m", "pytest", "tests/test_procedural_planning.py", "-q"),
        },
        {
            "id": "procedural_planning_autonomy_tests",
            "title": "Procedural Planning Autonomy Tests",
            "command": _python("-m", "pytest", "tests/test_companion_autonomy_runtime.py", "-k", "procedural", "-q"),
        },
        {
            "id": "procedural_planning_backend_readback_tests",
            "title": "Procedural Planning Backend Readback Tests",
            "command": _python(
                "-m",
                "pytest",
        ...
        {
            "id": "procedural_growth_phase2_smokes",
            "title": "Procedural Growth Phase 2 Smokes",
            "command": _python("evals/run_procedural_growth_phase2_smokes.py", "--run-tag", f"{run_id}-smokes"),
            "parser": "smokes",
        },
    ]
    ...
    *,
    ...
    if parser == "smokes":
        ...
        if smoke_status != "passed":
            status = "failed"
            failure_reasons.append("procedural_growth_phase2_smokes_overall_status=" + (smoke_status or "unknown"))
        if str(artifacts.get("failed") or "0") not in {"", "0"}:
            status = "failed"
            failure_reasons.append("procedural_growth_phase2_smokes_failed=" + str(artifacts.get("failed")))
    elif status != "passed":
...
def _aggregate_report(checks: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "readiness_status": (
            ...
            else "procedural_growth_phase2_in_progress"
        ),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        f"# Procedural Growth Phase 2 Audit ({report.get('run_id', 'unknown')})",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the Procedural Growth Phase 2 audit.")
    ...
    json_path = REPORT_DIR / f"procedural-growth-phase2-audit-{run_id}.json"
    md_path = REPORT_DIR / f"procedural-growth-phase2-audit-{run_id}.md"
    json_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    md_path.write_text(render_markdown(report), encoding="utf-8")
    print(f"[procedural-growth-phase2-audit] json={json_path}")
    print(f"[procedural-growth-phase2-audit] md={md_path}")
    print(f"[procedural-growth-phase2-audit] overall_status={report.get('overall_status', 'unknown')}")
    print(f"[procedural-growth-phase2-audit] readiness={report.get('readiness_status', 'unknown')}")
