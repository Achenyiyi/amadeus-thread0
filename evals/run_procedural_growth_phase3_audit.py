...
SMOKES_JSON_RE = re.compile(r"^\[procedural-growth-phase3-smokes\]\s+json=(.+)$", re.MULTILINE)
SMOKES_MD_RE = re.compile(r"^\[procedural-growth-phase3-smokes\]\s+md=(.+)$", re.MULTILINE)
SMOKES_STATUS_RE = re.compile(r"^\[procedural-growth-phase3-smokes\]\s+overall_status=(.+)$", re.MULTILINE)
    ...
    return [
        {
            ...
            "title": "Procedural Outcome Unit Tests",
            "command": _python("-m", "pytest", "tests/test_procedural_outcome.py", "-q"),
        },
        {
            "id": "procedural_outcome_planning_tests",
            "title": "Procedural Outcome Planning Tests",
            "command": _python("-m", "pytest", "tests/test_procedural_planning.py", "-k", "outcome", "-q"),
        },
        {
            "id": "procedural_outcome_backend_readback_tests",
            "title": "Procedural Outcome Backend Readback Tests",
            "command": _python(
                "-m",
                "pytest",
        ...
        {
            "id": "procedural_growth_phase3_smokes",
            "title": "Procedural Growth Phase 3 Smokes",
            "command": _python("evals/run_procedural_growth_phase3_smokes.py", "--run-tag", f"{run_id}-smokes"),
            "parser": "smokes",
        },
    ]
    ...
    *,
    ...
    if parser == "smokes":
        ...
        if smoke_status != "passed":
            status = "failed"
            failure_reasons.append("procedural_growth_phase3_smokes_overall_status=" + (smoke_status or "unknown"))
        if str(artifacts.get("failed") or "0") not in {"", "0"}:
            status = "failed"
            failure_reasons.append("procedural_growth_phase3_smokes_failed=" + str(artifacts.get("failed")))
    elif status != "passed":
...
def _aggregate_report(checks: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "readiness_status": (
            ...
            else "procedural_growth_phase3_in_progress"
        ),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        f"# Procedural Growth Phase 3 Audit ({report.get('run_id', 'unknown')})",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the Procedural Growth Phase 3 audit.")
    parser.add_argument("--run-tag", default="", help="Optional suffix for report filenames.")
    args = parser.parse_args()
    run_id = str(args.run_tag or "").strip() or f"phase3-audit-{uuid.uuid4().hex[:8]}"
    checks = [
        _check(
            spec["id"],
            spec["title"],
    ...
    json_path = REPORT_DIR / f"procedural-growth-phase3-audit-{timestamp}{suffix}.json"
    md_path = REPORT_DIR / f"procedural-growth-phase3-audit-{timestamp}{suffix}.md"
    json_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    md_path.write_text(render_markdown(report), encoding="utf-8")
    print(f"[procedural-growth-phase3-audit] json={json_path}")
    print(f"[procedural-growth-phase3-audit] md={md_path}")
    print(f"[procedural-growth-phase3-audit] overall_status={report['overall_status']}")
    print(f"[procedural-growth-phase3-audit] readiness_status={report['readiness_status']}")
