...
- These post-unlock ready statuses are bounded contracts, not blanket runtime authority:
  ...
  - embodied interaction consumes consent-bound source artifacts, approved artifact metadata, read-only appraisal evidence, and read-only motive/goal advisory hints derived from that metadata only; it does not create a live capture runtime
