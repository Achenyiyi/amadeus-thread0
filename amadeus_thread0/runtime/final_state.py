...
def digital_body_consequence_has_signal(consequence: dict[str, Any] | None) -> bool:
    ...
    return any(
        (
            ...
            str(consequence.get("browser_run_id") or "").strip(),
            str(consequence.get("browser_profile_id") or "").strip(),
            str(consequence.get("browser_page_id") or "").strip(),
            str(consequence.get("browser_tab_id") or "").strip(),
            str(consequence.get("browser_url") or "").strip(),
            str(consequence.get("browser_title") or "").strip(),
            str(consequence.get("browser_last_action_kind") or "").strip(),
            str(consequence.get("browser_last_exit_status") or "").strip(),
...
def resolve_digital_body_consequence(
    *,
    ...
    body_fill_raw = {
        ...
        "browser_run_id": _dict_or_empty(access_state.get("browser_runtime_state")).get("last_run_id"),
        "browser_profile_id": resource_state.get("browser_profile_id"),
        "browser_page_id": _dict_or_empty(access_state.get("browser_runtime_state")).get("active_page_id"),
        "browser_tab_id": resource_state.get("browser_tab_id"),
        "browser_url": resource_state.get("artifact_source_url"),
