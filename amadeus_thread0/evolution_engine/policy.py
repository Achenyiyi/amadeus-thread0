...
def build_behavior_policy(
    *,
    ...
    warmth = clamp01(0.28 + 0.28 * closeness + 0.18 * trust + 0.08 * clamp01(world.get("bond_depth"), 0.0) - 0.20 * hurt - 0.14 * boundary_pressure)
    sharpness = clamp01(0.18 + 0.30 * clamp01(tsundere_intensity, 0.55) + 0.18 * irritation + 0.10 * clamp01(world.get("boundary_load"), 0.0))
    initiative = clamp01(0.20 + 0.34 * engagement + 0.12 * clamp01(latent.get("agency_pressure"), 0.28) + 0.08 * cognitive_budget - 0.14 * autonomy_need - 0.10 * boundary_pressure)
    ...
    reply_length_bias = clamp01(0.30 + 0.24 * cognitive_budget + 0.12 * clamp01(world.get("task_pull"), 0.0) + 0.08 * clamp01(world.get("memory_gravity"), 0.0) - 0.10 * irritation)
    approach_vs_withdraw = clamp01(0.20 + 0.36 * engagement + 0.14 * clamp01(world.get("companionship_pull"), 0.0) - 0.16 * autonomy_need - 0.16 * hurt - 0.14 * boundary_pressure)
    ...
    self_directedness = clamp01(0.16 + 0.28 * autonomy_need + 0.18 * clamp01(world.get("agency_load"), 0.0) + 0.18 * clamp01(latent.get("agency_pressure"), 0.28))
