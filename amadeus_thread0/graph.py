...
"""Legacy compatibility facade for external graph imports.

Internal runtime code should import from ``amadeus_thread0.graph_parts`` directly.
This module only keeps the external compatibility surface and resolves legacy
helper symbols lazily.
"""
...
_LEGACY_EXPORT_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    (
        ".graph_parts.appraisal",
        (
    ...
    (
        ".graph_parts.counterpart_dynamics",
        (
    ...
    (
        ".graph_parts.generation_profile",
        (
    ...
    (
        ".graph_parts.prompt_helpers",
        (
            "_compact_interaction_carryover_hint",
        ),
    ),
    (
        ".graph_parts.prompting",
        (
            "_build_task_prompt",
        ),
    ),
    ...
    (
        ".graph_parts.semantic_narrative",
        (
...
_LEGACY_EXPORTS = {
    ...
    for module_name, names in _LEGACY_EXPORT_GROUPS
    for name in names
}

_default_persona_core = get_persona_core_authority

__all__ = tuple(_PUBLIC_EXPORTS) + tuple(_LEGACY_EXPORTS)
