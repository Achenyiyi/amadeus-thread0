...
- `operator_readback`, `dynamic_skill_candidate_runtime`, `living_loop_realism`, and `embodied_interaction` are backend-owned readbacks; frontend should render them as received and never derive memory, body, motive, autonomy, skill-registry, or causal-loop truth from them
...
This top-level block is a backend-owned readback for Dynamic Skill Candidate Runtime Phase 1.
...
- frontend may render the readback and the compact mirrors under `skills.dynamic_candidate_runtime` and `operator_readback.dynamic_skill_candidate_runtime`
- pending, blocked, rejected, drifted, or proposal-only candidates remain non-facts
- installed/active evidence comes from backend skill-registry/session truth, not frontend inference
- completed dynamic skill use is the only path that may resurface as procedural continuity
- this block does not authorize skill installation, automatic registry writes, prompt/model calls, memory writes, persona mutation, behavior mutation, live capture, frontend-owned semantics, or external mutation
