...
- `Advisor Demo Dry Run Phase 1` is now implemented as a scripted rehearsal/archive-readiness gate: `advisor_demo_dry_run.v1` composes ready Advisor Demo Readiness evidence with demo script, runbook, checklist, and manifest text; it proves the rehearsal path is checkable and archive-ready, not that a live advisor demo has already been observed, and adds no runtime authority, live capture, model calls, skill registry writes, external harness enablement, frontend-owned semantics, HTTP server ownership, persona mutation, memory writes, or external mutation.
...
- advisor demo dry-run gate: `python evals/run_advisor_demo_dry_run_phase1_audit.py`
...
39. `Advisor Demo Dry Run Phase 1` - `baseline-closed`
