...
class BackendApiTests(unittest.TestCase):
    ...
    def test_turn_and_event_responses_attach_artifact_semantics(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            checkpoint_db = root / "checkpoints.sqlite"
            checkpoint_db.write_bytes(b"x")
            ...
            for payload in (turn_payload, event_payload):
                ...
                self.assertEqual(payload["current_event"]["perception"]["channel"], "image")
                self.assertEqual(
                    payload["current_event"]["perception"]["semantic_observations"][0][
                        "source_ref_id"
                    ],
                    "img-backend-sem-1",
                )
                self.assertEqual(payload["turn_appraisal"]["scene"], "artifact_review")
                self.assertEqual(
                    payload["turn_appraisal"]["perception_semantics"]["semantic_observations"][0][
                        "source_ref_id"
                    ],
                    "img-backend-sem-1",
                )
                self.assertEqual(
                    payload["interaction_carryover"]["embodied_context"][
                        "artifact_semantic_observations"
                    ][0]["source_ref_id"],
                    "img-backend-sem-1",
                )
                self.assertEqual(observation["source_ref_id"], "img-backend-sem-1")
                self.assertIs(observation["model_api_called"], False)

    def test_turn_and_event_responses_attach_artifact_appraisal(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            checkpoint_db = root / "checkpoints.sqlite"
            checkpoint_db.write_bytes(b"x")
            ...
            state_values = {
                ...
                "current_event": {
                    ...
                    "digital_body_hints": {
                        "multimodal_sources": [
                            {
                                "source_id": "img-backend-appraisal-1",
                                "modality": "image",
                                "path": "fixtures/login.png",
                                "consent_scope": "single_turn",
                                "capture_method": "operator_attached_file",
            ...
            for payload in (turn_payload, event_payload):
                ...
                self.assertEqual(
                    payload["current_event"]["perception"]["appraisal_evidence"][0]["source_ref_id"],
                    "img-backend-appraisal-1",
                )
                self.assertEqual(
                    payload["turn_appraisal"]["artifact_evidence"][0]["source_ref_id"],
                    "img-backend-appraisal-1",
                )
                self.assertEqual(
                    payload["turn_appraisal"]["perception_semantics"]["appraisal_evidence"][0][
                        "source_ref_id"
                    ],
                    "img-backend-appraisal-1",
                )
    ...
    def test_turn_and_event_responses_attach_artifact_behavior_alignment(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            checkpoint_db = root / "checkpoints.sqlite"
            checkpoint_db.write_bytes(b"x")
            ...
            for payload in (turn_payload, event_payload):
                ...
                self.assertEqual(alignment["readiness_status"], "artifact_behavior_alignment_ready")
                self.assertEqual(
                    alignment["alignment_items"][0]["alignment_status"],
                    "advisory_not_reflected",
                )
                self.assertEqual(
                    payload["current_event"]["perception"]["behavior_alignment"]["alignment_items"][0][
                        "source_ref_id"
                    ],
                    "img-backend-align-1",
                )
                self.assertEqual(
                    payload["turn_appraisal"]["behavior_alignment_evidence"]["alignment_items"][0][
                        "source_ref_id"
                    ],
                    "img-backend-align-1",
                )
                self.assertEqual(
                    payload["turn_appraisal"]["perception_semantics"]["behavior_alignment"][
                        "alignment_items"
                    ][0]["source_ref_id"],
                    "img-backend-align-1",
                )
                ...
                self.assertEqual(payload["behavior_plan"]["primary_motive"], "continue_workspace_task")
                self.assertFalse(
                    payload["behavior_plan"]["artifact_behavior_alignment"]["alignment_summary"][
